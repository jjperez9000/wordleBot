...
responseChannel = 935546395237175408
...
async def print_leaderboard(message):
    with open('scores.json', 'r') as f:
        ...
        currentGuild = 935546369832284300
...
async def handle_submission(message):
    ...
    with open('scores.json', 'r') as f:
        ...
        if str(message.author.id) not in data:
            ...
            new_user = {message.author.id: {"score": 0, "playedToday": False, "completions": 0}}
        ...
        # if no fail then update score
            ...
            if int(message.content[12]) == 1:
                ...
                guess = " guesses"
            await channel.send("Today's score received: " + str(7 - int(message.content[12])) + " points for " + str(message.content[12]) + guess + ". " + str(message.author.nick) + "'s total: " + str(data[str(message.author.id)]["score"]))
